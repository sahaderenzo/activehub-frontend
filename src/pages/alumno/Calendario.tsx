...
export default function AlumnoCalendario() {
    ...
    const [ahora] = useState<number>(() => Date.now());
    ...
    const proximas = useMemo(() => {
        return inscripciones
            .filter(
                (i) =>
                    i.estado !== "Cancelada" &&
                    new Date(i.claseFechaHora).getTime() > ahora,
            )
            .slice()
    ...
    const itemsDelDiaSeleccionado =
        celdas.find((c) => c.key === diaSeleccionado)?.items ?? [];
    const fechaSeleccionada = celdas.find(
        (c) => c.key === diaSeleccionado,
    )?.fecha;

    return (
        <div
            className="ah-screen"
            style={s("min-height:100vh;background:#F4F7FA;")}
        >
            <AlumnoNav active="calendario" />
            <div
                style={s(
                    "max-width:900px;margin:0 auto;padding:30px 28px 60px;",
                )}
            >
                <div
                    style={s(
                        "display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:22px;",
                    )}
                >
                    <div>
                        <h1
                            style={s(
                                "font:700 30px Space Grotesk,sans-serif;letter-spacing:-.7px;margin:0 0 4px;",
                            )}
                        >
                        ...
                        <p
                            style={s(
                                "font-size:14.5px;color:#7A8C9E;margin:0;",
                            )}
                        >
                            Tus próximas clases y preinscripciones, agrupadas
                            por fecha.
                        </p>
                    </div>
                    <div
                        style={s(
                            "display:flex;gap:4px;background:#EEF1F4;border-radius:11px;padding:4px;flex:none;",
                        )}
                    >
                        <button
                            className="ah-btn"
                ...
                {proximas.length === 0 && (
                    ...
                    >
                        ...
                        <Link
                            ...
                            className="ah-link"
                            style={s(
                                "color:#FF6A2B;font-weight:700;text-decoration:none;",
                            )}
                        >
                            Explorá actividades →
                        </Link>
                    ...
                    >
                        {grupos.map(([fecha, items]) => (
                            <div key={fecha}>
                                <div
                                    style={s(
                                        "font:700 13px Manrope,sans-serif;color:#FF6A2B;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;",
                                    )}
                                >
                                    {fecha}
                                </div>
                                ...
                                >
                                    {items.map((item) => (
                                        <ClaseCard
                                            ...
                                            onClick={() =>
                                                navigate(
                                                    `/alumno/actividad/${item.actividadId}`,
                                                )
                                            }
                    ...
                    <div>
                        ...
                        >
                            ...
                            >
                                ...
                                <span
                                    style={s(
                                        "font:700 16px Space Grotesk,sans-serif;color:#0E2A47;min-width:170px;text-align:center;",
                                    )}
                                >
                                    {etiquetaMes}
                                </span>
                            ...
                            >
                                {DIAS_SEMANA.map((d) => (
                                    <div
                                        key={d}
                                        style={s(
                                            "text-align:center;font:700 11px Manrope,sans-serif;color:#9AAABA;text-transform:uppercase;letter-spacing:.4px;padding:4px 0;",
                                        )}
                                    >
                                        {d}
                                    </div>
                                ...
                                {celdas.map((c) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span
                                                style={s(
                                                    `font:700 12.5px Space Grotesk,sans-serif;color:${on ? "#fff" : "#0E2A47"};`,
                                                )}
                                            >
                                                {c.fecha.getDate()}
                                            </span>
                                                    ...
                                                    {c.items
                                                        ...
                                                        .map((it) => (
                                                            <span
                                                                key={it.id}
                                                                style={s(
                                                                    `width:6px;height:6px;border-radius:99px;background:${on ? "#fff" : "#FF6A2B"};`,
                                                                )}
                                                            />
                                                        ))}
                                                </div>
                        ...
                        <div
                            style={s(
                                "font:700 13px Manrope,sans-serif;color:#7A8C9E;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;",
                            )}
                        >
                            {fechaSeleccionada
                                ? formatFecha(fechaSeleccionada.toISOString())
                                : "Elegí un día"}
                        </div>
                        {itemsDelDiaSeleccionado.length === 0 ? (
                            <div
                                style={s(
                                    "background:#fff;border:1px dashed #D6DEE7;border-radius:14px;padding:26px 18px;text-align:center;color:#9AAABA;font-weight:600;font-size:13.5px;",
                                )}
                            >
...
function ClaseCard({
    ...
    const tipo = actividad
        ? getTipoActividad(actividad.tipoActividadId)
        : undefined;
    const cat = tipo ? getCategoria(tipo.categoriaId) : undefined;
    const instructor = actividad
        ? instructorNombre[actividad.instructorId]
        : undefined;
    return (
        <div
            className="ah-hov"
        ...
        >
            ...
            >
                <span
                    style={s(
                        "font:700 15px Space Grotesk,sans-serif;color:#0E2A47;",
                    )}
                >
                    {formatHora(item.claseFechaHora)} hs
                </span>
                <StatusBadge type={inscripcionStatusType(item.estado)} />
            </div>
            <div
                style={s(
                    "font:700 14.5px Manrope,sans-serif;color:#0E2A47;margin-bottom:3px;",
                )}
            >
                {item.actividadNombre}{" "}
                <span style={s("color:#9AAABA;font-weight:600;")}>
                    · {cat?.nombre}
                </span>
            </div>
            <div
                style={s(
                    "font-size:12.5px;color:#7A8C9E;font-weight:600;margin-bottom:9px;",
                )}
            >
                {instructor ?? ""}
            </div>
            {actividad && (
                <div
                    style={s(
                        "display:flex;align-items:center;justify-content:flex-end;font-size:13px;",
                    )}
                >
                    <span
                        style={s(
                            "font:700 14px Space Grotesk,sans-serif;color:#0E2A47;",
                        )}
                    >
